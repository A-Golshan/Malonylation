...
def ExtractFeatures(df, window_size=5, path='train'):
    ...
    for i, fasta in enumerate(sequences):
        ...
        pssm_df = loadPSSM(f'pssm/{path}/{i + 1}.pssm')
        for fragment in fragments:
            ...
            if fragment[1] in pos_index:
                FeatureMatrix_POS.append(np.concatenate((eaac, egaac, blo, pssm, [1])))
            else:
                FeatureMatrix_NEG.append(np.concatenate((eaac, egaac, blo, pssm, [0])))
...
if __name__ == '__main__':
    ...
    FM_train.to_csv(f'FM_train_{WINDOW_SIZE}.csv', index=False)
    ...
    FM_test.to_csv(f'FM_Test_{WINDOW_SIZE}.csv', index=False)
    del FM_test
